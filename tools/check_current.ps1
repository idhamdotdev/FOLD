    ...
    public static void ShowCurrent() {
        ...
        while (EnumDisplayDevices(null, id++, ref dev, 0)) {
            if ((dev.StateFlags & 1) != 0) { // Active
                ...
                if (EnumDisplaySettings(dev.DeviceName, -1, ref mode)) { // -1 is ENUM_CURRENT_SETTINGS
                    Console.WriteLine(dev.DeviceName + " (" + dev.DeviceString + ") is at: " + mode.dmPelsWidth + "x" + mode.dmPelsHeight + "@" + mode.dmDisplayFrequency + "Hz");
                }
            }
        }
    }
